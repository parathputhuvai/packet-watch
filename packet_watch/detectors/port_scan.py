...
class PortScanDetector(Detector):
    ...
    def detect(self, packet, state):
        t = self.cfg
        n = len(state.scan_destination_ports)
        if n < t['distinct_destination_ports']:
            return None
        return DetectionResult(
            ...
            packet.dst_ip,
            packet.protocol,
            {'reason': 'Multiple distinct request-side destination ports contacted within the rolling window.'},
            {'distinct_destination_ports': n},
            t,
            mitre_for('port_scanning'),
        )
